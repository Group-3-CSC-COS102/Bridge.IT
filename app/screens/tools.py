...
class ToolsScreens:
    ...
    def create_tools_screen(self):
        ...
        #page header
        tk.Label(screen, text=f"AI Tools", bg=APP_BG_CLR, fg=APP_TITLE_TXT_CLR, font=TITLE_FONT).pack(pady=(10,6))
    ...
    def get_tools(self):
        ...
        for index, row in df.iterrows():
            ...
            if not title and not content:
                continue
            self.tools.append({"section_id": section_id, "title": title, "content": content})

    #Loads current tool section into the UI
    def current_section(self):
        #Failsafe if self.tools is empty
        if not self.tools:
